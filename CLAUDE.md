...
- **Styling:** Tailwind CSS. Use the design tokens defined in `talento-spec/02-DESIGN-SYSTEM.md` — never invent colours. If a hex is not in the palette, stop and ask.
...
The HTML files in `talento-spec/designs/` are the brief. Match them visually. When in doubt, open the HTML, inspect the computed styles, translate to Tailwind. Do not substitute "nicer" or "more modern" alternatives — the cinematic burnt-orange aesthetic is the brand.
