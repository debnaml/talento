...
import { saveTalent, unsaveTalent } from "@/app/studio/talent/actions";
...
export default async function TalentProfilePage({
  ...
  if (await isBlocked(user.id, id)) notFound();
  ...
  async function handleUnsave() {
    "use server";
    await unsaveTalent(id);
  }
  async function handleMessage() {
    "use server";
    await openAndRedirect(id, null, "studio");
  }
  async function handleBlock() {
    "use server";
    await blockUser(id);
    redirect("/studio/browse");
  }
      ...
      <div className="relative bg-dark-2 overflow-hidden">
        ...
        <div className="relative z-20 px-6 md:px-12 py-16 max-w-[580px]">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <form action={isSaved ? handleUnsave : handleSave}>
              <button
                type="submit"
                className={`font-condensed text-[13px] font-bold uppercase tracking-[2px] px-6 py-3 border transition-colors ${
                  isSaved
                    ? "bg-orange/10 border-orange text-orange hover:bg-orange hover:text-white"
                    : "bg-orange border-orange text-white hover:bg-orange-hot"
                }`}
              >
            ...
            <form action={handleMessage}>
              <button
                type="submit"
                className="font-condensed text-[13px] font-bold uppercase tracking-[2px] px-6 py-3 border border-white/20 text-warm-white hover:border-orange hover:text-orange transition-colors"
              >
                Message
              </button>
            </form>
            <form action={handleBlock}>
              <button
                type="submit"
                className="font-condensed text-[12px] font-bold uppercase tracking-[2px] px-4 py-3 text-grey hover:text-orange transition-colors"
                title="Block this talent"
              >
                Block
              </button>
            </form>
